...
export const Inscription = () => {
  ...
  const handleRegister = async(e:any)=> {
    ...
    const descriptionError = document.querySelector(".description.error") as HTMLInputElement
    const usedLanguageError = document.querySelector(".usedLanguage.error") as HTMLInputElement
    ...
    if(password !== passwordConfError.value || password.length < 8 || !terms.checked) {
      ...
      .then((res:any)=> {
        if (res.data.errors){
          ...
          usedLanguageError.innerHTML = res.data.errors.usedLanguage  
        ...
        {formSubmit ? (
          ...
          <form action='' onSubmit={handleRegister} className="bg-slate-100 flex
           ...
           my-4 md:mx-[20%] lg:mx-[20%]">
              ...
              <label htmlFor='description' className='mt-2 text-center text-black font-serif'>Votre description</label>
              <input className='border-2 border-black mx-12 sm:mx-52 md:mx-[30%]
              lg:mx-[30%]' placeholder="Vous pouvez ici vous présenter" 
              type="text" name='description' value={description} 
              onChange={(e)=> setDescription(e.target.value)}/>
              <div className='description error'></div>

              <label htmlFor='usedLanguage' className='mt-2 text-center text-black font-serif'>Votre stack</label>
              <input className='border-2 border-black mx-12 sm:mx-52 md:mx-[30%]
              lg:mx-[30%]' placeholder="Vous pouvez ici préciser votre stack" 
              type="text" name='usedLanguage' value={description} 
              onChange={(e)=> setUsedLanguage(e.target.value)}/>
              <div className='usedLanguage error'></div>
